...
(define-data-var commission-address principal 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM)
  ...
  (let 
    ...
    (match (map-get? verified-contracts {nft-asset-contract: asset-contract})
      ...
      (begin
        ...
        (print 
          {
            type: "froggy-auctions-v1",;; "stxnft-auctions-v1",
            action: "list-auction",
...
(define-private (transfer-nothing (amount uint) (recipient principal) )
  (contract-call? .nope transfer amount tx-sender recipient none)
)
...
(define-private (transfer-nothing-as-contract (amount uint) (recipient principal) )
  (as-contract (contract-call? .nope transfer amount tx-sender recipient none))
)
...
(define-public (place-bid (nft-asset-contract <nft-trait>) (token-id uint) (amount uint))
  (let
    ...
    ;; Froggys bid in $NOT
    ;; (try! (contract-call? .nope transfer amount tx-sender (as-contract tx-sender)))
    ...
    (if (> (get amount current-bid) u0)
      (begin 
        ...
        ;; (try! (as-contract (contract-call? .nope transfer (get amount current-bid) tx-sender (get buyer current-bid))))
        ;; use transfer-nothing-as-contract 
  ...
  (let 
    ...
    (if (>= (get amount current-bid) (get reserve-price auction))
      (begin
        ...
        ;; transfer $NOT of successful bid from contract to seller, minus commission and royalty
        ...
        ;; (try! (as-contract (stx-transfer? commission tx-sender (var-get commission-address))))
        ;; transfer commission to auction house in $NOT
        ;; (try! (as-contract (contract-call? .nope transfer commission tx-sender (var-get commission-address))))
        ;; do it using the transfer-nothing-as-contract private function
        ...
        ;; transfer royalty to artist in $NOT
        ;; (try! (as-contract (contract-call? .nope transfer royalty tx-sender (get royalty-address auction)))
        ;; do it using the transfer-nothing-as-contract private function
...
(define-public (admin-remove-bid (nft-asset-contract <nft-trait>) (token-id uint))
  (let 
    ...
    ;; (try! (as-contract (stx-transfer? (get amount current-bid) tx-sender (get buyer current-bid))))
    ;; do it with transfer-nothing-as-contract
    (try! (transfer-nothing-as-contract (get amount current-bid) (get buyer current-bid)))
    (map-delete auction-bids {nft-asset-contract: (contract-of nft-asset-contract), token-id: token-id})
