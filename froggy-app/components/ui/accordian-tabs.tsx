...
export const AccordionTabs = ({ tabsContentArray, initialTab, className, ...rest }: CombinedProps) => {
  ...
  return (
    <div
      ...
      {...rest}
    >
      <div className="flex w-full items-center justify-between bg-purple-300 font-bold rounded-t-md">
        {tabsContentArray.map(({ label }, index) => (
          <button
            ...
            className={cn(
              "flex items-center justify-center cursor-pointer flex-1 border-2 border-black -m-[2px] max-sm:px-2 px-6 py-3 text-center font-bold transition-colors first:rounded-tl-md",
              {
                "bg-purple-400": activeTab === label && showContent,
                "bg-purple-300": activeTab !== label || !showContent,
              }
            )}
          >
            {label}
